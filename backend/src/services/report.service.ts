    ...
    try {
      ...
      const check1 = await transaction.request()
        ...
        .query(`
          ...
          WHERE BookedAt BETWEEN @start AND @end
        `);
      ...
      // Load report group items
      let queryGroupCol = "BookedAt";
      if (reportType === "cinema") queryGroupCol = "CinemaName";
      else if (reportType === "movie") queryGroupCol = "MovieTitle";
      ...
      const groupQuery = await transaction.request()
        ...
        .query(`
          SELECT 
            ${queryGroupCol} as Label,
            SUM(TotalPrice) as Revenue,
            COUNT(*) as TicketsCount
          FROM Ticket
          WHERE BookedAt BETWEEN @start AND @end
          GROUP BY ${queryGroupCol}
        `);
